...
static OpcodeEntry opcode_table [512] = {
  ...
  /* 0x30 */	EMPTY, IDEXW(G2E, xor, 1), EMPTY, EMPTY,
  ...
  /* 0x54 */	EMPTY, IDEX(push_EBX, push), EMPTY, EMPTY,
  /* 0x58 */	EMPTY, EMPTY, EMPTY, EMPTY,
  /* 0x5c */	EMPTY, EMPTY, EMPTY, EMPTY,
  /* 0x60 */	EMPTY, EMPTY, EMPTY, EMPTY,
  /* 0x64 */	EMPTY, EMPTY, EX(operand_size), EMPTY,
  /* 0x68 */	IDEXW(push_SI, push, 4), EMPTY, EMPTY, EMPTY,
  ...
  /* 0xf0 */	EMPTY, EMPTY, EMPTY, IDEXW(D, dop, 3),
...
void isa_exec(vaddr_t *pc) {
  uint32_t opcode = instr_fetch(pc, 1);  // read the opcode from memory(opcode is always the first byte)
  decinfo.opcode = opcode;
  set_width(opcode_table[opcode].width);
  idex(pc, &opcode_table[opcode]);
}
